      ...
      <DashboardLayout>
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-extrabold text-gray-800 mb-6 flex items-center">
            <QrCode className="mr-3" /> Scanner La Manada
          </h1>

          {/* Barra de controle */}
          <div className="bg-white p-4 rounded-lg shadow mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {!scanning ? (
                <button onClick={start} className="flex items-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
                  <Camera size={18} className="mr-2" /> Ligar câmera
                </button>
              ) : (
                <button onClick={stop} className="flex items-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
            ...
            {/* Manual */}
            <form onSubmit={submitManual} className="flex gap-2 items-center">
              <input
                value={manual}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {history.map((h, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      <td className="px-6 py-3 text-sm text-gray-700">{h.ts}</td>
                      <td className="px-6 py-3 text-sm text-gray-700">{h.value}</td>
                      <td className="px-6 py-3 text-sm text-gray-700">{h.name}</td>
